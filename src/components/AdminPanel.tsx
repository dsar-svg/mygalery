...
import { Plus, Trash2, Edit3, Save, X, LogIn, Image as ImageIcon, Upload, Printer, QrCode, Settings, Sparkles } from 'lucide-react';
...
export function AdminPanel({ user }: AdminPanelProps) {
  ...
  if (!user) {
    return (
      <div className="py-24 flex flex-col items-center justify-center space-y-12 max-w-lg mx-auto text-center">
        <div className="w-24 h-24 bg-bone-dark rounded-full flex items-center justify-center shadow-inner">
                  ...
                  <div className="flex flex-col md:flex-row gap-6 items-start">
                    {settingsData.heroImageUrl && (
                      <div className="w-40 h-24 rounded-xl overflow-hidden bg-bone shadow-inner flex-shrink-0">
                        <img src={settingsData.heroImageUrl} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                      </div>
                    )}
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-16">
              ...
              <div className="space-y-8 flex flex-col h-full justify-between">
                <div className="space-y-4">
                  <label className="text-[9px] font-black uppercase tracking-[0.3em] opacity-30 px-1">Imagen de la Obra y Calidad Visual</label>
                  <div className="space-y-4">
        ...
        {artworks.map((art) => (
          ...
          >
            <div className="w-32 h-32 rounded-2xl overflow-hidden bg-bone-dark flex-shrink-0 shadow-inner">
               {art.imageUrl ? (
                 <img src={art.imageUrl} className="w-full h-full object-cover grayscale-[0.5] group-hover:grayscale-0 transition-all duration-700" referrerPolicy="no-referrer" />
               ) : (
