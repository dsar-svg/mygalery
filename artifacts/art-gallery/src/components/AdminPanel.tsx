...
export function AdminPanel({ user }: AdminPanelProps) {
  ...
  useEffect(() => {
    // Usamos los nombres correctos del servicio
  ...
  const handleUpdateSettings = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      // Ahora el servicio devuelve los settings actualizados
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-charcoal/10 pb-12">
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            className="flex items-center gap-2 px-6 py-4 rounded-full bg-white border border-charcoal/10 text-[10px] font-bold uppercase tracking-widest hover:bg-charcoal hover:text-white transition-all shadow-sm text-charcoal"
          >
      ...
      <AnimatePresence>
        {showAccess && (
          <motion.div
            ...
            className="overflow-hidden bg-white rounded-[2rem] p-8 border border-charcoal/5 shadow-sm space-y-8"
          >
             <div className="flex items-center gap-3 border-b border-charcoal/5 pb-4">
                <Mail className="w-5 h-5 opacity-40 text-charcoal" />
              ...
              <div className="flex gap-4">
                <input
                  ...
                  className="flex-1 bg-bone-light border border-charcoal/10 rounded-2xl p-4 outline-none focus:border-charcoal/30 transition-all text-sm text-charcoal"
                />
                <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {allowedEmails.map(email => (
                  <div key={email} className="flex items-center justify-between p-4 bg-bone-light rounded-xl border border-charcoal/5 group">
                    <span className="text-xs font-medium opacity-60 text-charcoal">{email}</span>
                    <button
        ...
        {showSettings && settings && (
          <motion.div
            ...
            animate={{ height: 'auto', opacity: 1 }}
            exit={{ height: 0, opacity: 0 }}
            className="overflow-hidden bg-white rounded-[2rem] p-8 border border-charcoal/5 shadow-sm"
          >
            <form onSubmit={handleUpdateSettings} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-charcoal">
              ...
              <div className="flex justify-end pt-6 border-t border-charcoal/5">
                <button
                  type="submit"
                  disabled={isSaving}
                  className="bg-charcoal text-white px-12 py-4 rounded-full text-[10px] font-bold uppercase tracking-widest hover:opacity-90 transition-all disabled:opacity-50"
                >
          ...
          >
            <form onSubmit={handleAddArt} className="bg-white rounded-[3rem] p-8 md:p-12 border border-charcoal/5 shadow-2xl space-y-12">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div className="space-y-8 text-charcoal">
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      className="w-full bg-bone-light border border-charcoal/10 rounded-2xl p-5 outline-none focus:border-charcoal/30 transition-all h-32 resize-none text-sm"
                    />
                  </div>
                ...
                <div className="space-y-6">
                  <label className="text-[10px] font-bold uppercase tracking-widest opacity-30 text-charcoal">Imagen de la Obra</label>
                  <div className="relative aspect-square rounded-[2rem] border-2 border-dashed border-charcoal/10 bg-bone-light flex flex-col items-center justify-center overflow-hidden">
                    {newArt.imageUrl ? (
                      <>
                        ...
                        <button
                          ...
                          className="absolute top-4 right-4 bg-charcoal text-white p-2 rounded-full"
                        >
                          <X className="w-4 h-4" />
                        </button>
                      </>
                      ...
                      <div className="text-center space-y-4">
                        <Upload className="w-8 h-8 opacity-20 mx-auto text-charcoal" />
                        <p className="text-[10px] font-bold uppercase tracking-widest opacity-40 text-charcoal">Subir Archivo</p>
                        <input
                          type="file"
                          accept="image/*"
              ...
              <div className="flex justify-end pt-8 border-t border-charcoal/5">
                <button
                  ...
                  className="bg-charcoal text-white px-16 py-6 rounded-full text-[10px] font-bold uppercase tracking-widest hover:scale-105 transition-all shadow-2xl disabled:opacity-50"
                >
        ...
        <div className="overflow-x-auto">
                ...
                <tr key={art.id} className="group hover:bg-bone-light/30 transition-colors">
                  <td className="p-8">
                    <div className="flex items-center gap-6">
                      <div className="w-16 h-16 rounded-xl overflow-hidden bg-bone-light border border-charcoal/5">
                      ...
                      {editingId === art.id ? (
                        <input
                          ...
                          className="bg-bone-light border border-charcoal/10 rounded-lg p-2 font-serif italic text-charcoal"
                        />
                      ) : (
                  ...
                  <td className="p-8 text-charcoal">
                    {editingId === art.id ? (
                      <input
                        type="text"
                      ...
                      {editingId === art.id ? (
                        <>
                          ...
                          <button
                            ...
                            className="p-3 bg-bone-light rounded-full hover:scale-110 transition-all text-charcoal"
                          >
                            <X className="w-4 h-4" />
                          </button>
                        </>
          ...
          {artworks.length === 0 && (
            <div className="p-20 text-center space-y-4">
              <ImageIcon className="w-8 h-8 opacity-10 mx-auto text-charcoal" />
              <p className="font-serif italic text-xl opacity-30 text-charcoal">No hay obras en la colección todavía.</p>
            </div>
          )}
        </div>
      </div>
          ...
          >
            ...
            >
              <div className="space-y-4 text-charcoal">
                ...
                <p className="text-[10px] uppercase tracking-widest opacity-40">Escanea para ver detalles</p>
              </div>
              ...
              <div className="flex gap-4">
                ...
                <button
                  ...
                  className="w-16 h-16 border border-charcoal/10 rounded-full flex items-center justify-center hover:bg-charcoal hover:text-bone-light transition-all text-charcoal"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
